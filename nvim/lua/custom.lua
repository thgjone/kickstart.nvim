...
function _G.VimDML(choise)
	...
	--- local custom_env = { FT= vim.fn.expand("&filetype") }
	--- 
	--- print(vim.fn.environ())

	--- -- Merge with existing environment (vim.env)
	--- local full_env = vim.tbl_extend("force", vim.fn.environ(), custom_env)

	--- local result = vim.system(
	--- 	{ '_dml_composer',  '--browse-prg',  'fmenu', '-r', '--history', '10', '-i', '"' .. dmlfile .. '"' }, {env = full_env}, 
	--- 	function (obj) 
	--- 		print(obj.stdout)
	--- 	end
	--- );
